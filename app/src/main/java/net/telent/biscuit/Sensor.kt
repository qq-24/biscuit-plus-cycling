...
class PositionSensor(onStateChange: (s: Sensor) -> Unit) : ISensor , Sensor("location", onStateChange) {
    ...
    // GPS parameters read from SharedPreferences
    private var gpsAccuracyFilter: Boolean = false
    private var gpsAccuracyThreshold: Float = 20f
    private var gpsLowSpeedFilter: Boolean = false
    private var gpsLowSpeedThreshold: Float = 3f

    // Mock GPS mode
    private var mockMode: Boolean = false
    private var mockServerThread: Thread? = null
    private var mockServerSocket: java.net.ServerSocket? = null
    ...
    override fun startSearch(context: Context, deviceNumber: Int) {
        ...
        // Read GPS parameters from SharedPreferences
        val prefs = context.getSharedPreferences("biscuit_settings", Context.MODE_PRIVATE)
        val gpsInterval = prefs.getInt("gps_interval", 1000).toLong()
        val gpsMinDistance = prefs.getFloat("gps_min_distance", 0f)
        gpsAccuracyFilter = prefs.getBoolean("gps_accuracy_filter", false)
        gpsAccuracyThreshold = prefs.getFloat("gps_accuracy_threshold", 20f)
        gpsLowSpeedFilter = prefs.getBoolean("gps_low_speed_filter", true)
        gpsLowSpeedThreshold = prefs.getFloat("gps_low_speed_threshold", 3f)

        // Check mock mode
        mockMode = prefs.getBoolean("debug_mock_gps", false)
        if (mockMode && BuildConfig.ENABLE_MOCK_GPS) {
            // If mock server is already running, don't restart it (would kill active connections)
            if (mockRunning && mockServerThread?.isAlive == true) {
                Log.d("PositionSensor", "Mock server already running, skipping restart")
                state = ISensor.SensorState.PRESENT
                return
            }
            stopMockServer()  // Clean up any previous instance
            startMockServer()
            return
        }
        ...
        timestamp = Instant.now()
        locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, gpsInterval, gpsMinDistance, locListener)
    }
    ...
    private val locListener = object : LocationListener {
        ...
        override fun onLocationChanged(loc: Location) {
            // Accuracy filter: discard points where accuracy exceeds threshold
            if (gpsAccuracyFilter && loc.hasAccuracy() && loc.accuracy > gpsAccuracyThreshold) {
                Log.d(this.javaClass.name, "discarding location: accuracy ${loc.accuracy} > threshold $gpsAccuracyThreshold")
                return
            }

            state = ISensor.SensorState.PRESENT
            sensorName = loc.provider ?: "GPS"
            ...
            if (loc.hasSpeed()) {
                ...
                val timeDiff = (loc.time - lastLocation!!.time) / 1000.0 // 秒
                if (timeDiff > 0 && dist > 0) {
                    speed = (dist / timeDiff) * 3.6
                }
            }

            // Low speed filter: treat speed below threshold as 0, and skip distance accumulation
            if (gpsLowSpeedFilter && speed < gpsLowSpeedThreshold) {
                speed = 0.0
            } else {
                // 计算累计距离（仅在速度未被过滤时累加）
            ...
            lastLocation = loc
            Log.d(this.javaClass.name, "locationChanged $loc, speed: $speed, dist: $distance")
        }
    }
    ...
    @Volatile private var mockRunning = false
    ...
    private fun startMockServer() {
        ...
        timestamp = Instant.now()
        mockRunning = true
        Log.d("PositionSensor", "Mock GPS mode: starting TCP server on port 7777")

        mockServerThread = Thread {
            var serverSock: java.net.ServerSocket? = null
            try {
                serverSock = java.net.ServerSocket().apply {
                    reuseAddress = true
                    bind(java.net.InetSocketAddress("127.0.0.1", 7777))
                    soTimeout = 0  // block indefinitely on accept
                }
                mockServerSocket = serverSock
                Log.d("PositionSensor", "Mock server listening on port 7777")
                while (mockRunning && !Thread.currentThread().isInterrupted) {
                    val client = try {
                        serverSock.accept()
                    } catch (e: java.net.SocketException) {
                        if (!mockRunning) break  // intentional shutdown
                        Log.w("PositionSensor", "Mock accept error: ${e.message}")
                        continue
                    } catch (e: Exception) {
                        Log.w("PositionSensor", "Mock accept error: ${e.message}")
                        if (!mockRunning) break
                        Thread.sleep(500)
                        continue
                    }
                    Log.d("PositionSensor", "Mock client connected: ${client.remoteSocketAddress}")
                    try {
                        client.keepAlive = true
                        client.tcpNoDelay = true
                        client.soTimeout = 0  // no read timeout
                        ...
                        while (mockRunning) {
                            val line = try {
                                ...
                                Log.d("PositionSensor", "Mock read error: ${e.message}")
                                null
                            }
                            if (line == null) {
                                Log.d("PositionSensor", "Mock client disconnected (EOF)")
                                break
                            }
                            ...
                            try {
                                ...
                                val newSpeed = json.getDouble("speed")

                                if (latitude != null && longitude != null) {
                                    val results = FloatArray(1)
                                    Location.distanceBetween(latitude!!, longitude!!, newLat, newLng, results)
                                    if (results[0] > 0.01f) {
                                        distance += results[0]
                                    }
                                }
                                ...
                                Log.d("PositionSensor", "Mock: lat=$latitude lng=$longitude speed=$speed dist=$distance")
                            } catch (e: Exception) {
                                Log.w("PositionSensor", "Mock parse error for line '$line': ${e.message}")
                            }
                        }
                    } catch (e: Exception) {
                        Log.w("PositionSensor", "Mock client session error: ${e.message}")
                    } finally {
                        try { client.close() } catch (_: Exception) {}
                    }
                    if (mockRunning) {
                        Log.d("PositionSensor", "Waiting for next mock client...")
                    }
                }
            } catch (e: Exception) {
                Log.e("PositionSensor", "Mock server fatal error: ${e.message}", e)
            } finally {
                try { serverSock?.close() } catch (_: Exception) {}
                Log.d("PositionSensor", "Mock server thread exiting")
            }
        }.apply {
            isDaemon = true
            name = "MockGPS-Server"
    ...
    private fun stopMockServer() {
        mockRunning = false
        try { mockServerSocket?.close() } catch (_: Exception) {}
